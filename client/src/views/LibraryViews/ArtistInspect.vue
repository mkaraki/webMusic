...
let items = ref([]);
let artistInfo: Ref<any> = ref(null);
...
onMounted(() => {
    addItemsRecursive(`/library/${libraryId}/artist/${artistId}`);
});
...
<template>
    <div class="container-fluid">
        <div class="row mb-3" v-if="artistInfo !== null">
            <h2 class="display-1">{{ artistInfo['artistNames'][0] }}</h2>
            <div v-if="artistInfo['artistNames'].length > 1">
                <span v-for="name in artistInfo['artistNames'].slice(1)" :key="name">{{ name }}, </span> etc...
            </div>
        </div>
